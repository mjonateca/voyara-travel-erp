...
const statusLabels = { confirmed: "Confirmado", requested: "Solicitado", pending: "Pendiente", cancelled: "Cancelado" } as const;
export function StatusBadge({ status }: { status: keyof typeof statusMap }) { const [Icon, tone] = statusMap[status]; return <span className={cn("inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-[11px] font-semibold", tone)}><Icon size={12} />{statusLabels[status]}</span>; }
...
export function ErrorState({ reference }: { reference: string }) { return <GlassPanel className="border-rose-500/30 p-5"><p className="font-medium">No se pudo cargar esta sección.</p><p className="mt-1 text-sm text-muted">Vuelve a intentarlo en unos momentos. Referencia: {reference}</p></GlassPanel>; }
